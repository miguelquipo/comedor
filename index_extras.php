...
<button class="btn-return btn btn-primary" onclick="window.location.href='index.php'">
    ...
    <div class="container">
        <div class="header">
            ...
            <h2>Extras Comedor</h2>
        </div>
        <form id="registroForm" method="post" action="./PHP/registro_extras.php">
...
// Manejo de alertas según los parámetros en la URL
...
if (urlParams.get('success') === 'false') {
    ...
    switch (error) {
        ...
        case 'saldo_insuficiente':
            errorMessage = 'No tiene saldo suficiente para realizar la compra.';
            break;
        case 'saldo_no_encontrado':
            errorMessage = 'Esta persona no tiene un valor de saldo registrado.';
            break;
        case 'consumo_excede':
            errorMessage = 'No tiene saldo suficiente para realizar la compra.';
            break;
    ...
    Swal.fire({
        ...
        if (cedulaTemp) {
            window.location.href = `index_extras.php?cedula_temp=${cedulaTemp}`;
        }
    });
}
...
// Alerta de registro exitoso
    ...
    // Obtener nombre, apellido y saldo de la URL
    const nombreEmp = urlParams.get('nombre_emp') || 'Empleado';
    const apellidoEmp = urlParams.get('apellido_emp') || '';
    const saldo = urlParams.get('saldo') || '0.00';

    // Construir el mensaje de éxito
    const successMessage = `Registro exitoso para ${nombreEmp} ${apellidoEmp}.       Saldo actual: $${saldo}.`;
...
function confirmarPago() {
    Swal.fire({
        ...
        text: '¿Deseas proceder con esta compra?',
